...
BEGIN
    ...
    -- Covering index for SP_PROCESS_BATCH JOIN on BATCH_ID (eliminates key lookups)
    CREATE NONCLUSTERED INDEX [IX_MONERIS_BATCH_COVERING] ON [dbo].[MONERIS_TOKENS_STAGING] ([BATCH_ID])
    INCLUDE ([MONERIS_TOKEN], [EXP_DATE], [ENTITY_ID], [ENTITY_TYPE], [LAST_USE_DATE]);

    -- Filtered index for pending validation queries
    CREATE NONCLUSTERED INDEX [IX_MONERIS_PENDING_VALIDATION] ON [dbo].[MONERIS_TOKENS_STAGING] ([FILE_ID], [VALIDATION_STATUS])
    INCLUDE ([MONERIS_TOKEN], [EXP_DATE], [ENTITY_ID], [ENTITY_TYPE], [ENTITY_STS])
    WHERE [VALIDATION_STATUS] = 'PENDING';

    -- Filtered index for valid tokens awaiting batch assignment
    CREATE NONCLUSTERED INDEX [IX_MONERIS_VALID_NOBATCH] ON [dbo].[MONERIS_TOKENS_STAGING] ([FILE_ID], [VALIDATION_STATUS])
    INCLUDE ([MONERIS_TOKEN], [BATCH_ID])
    WHERE [VALIDATION_STATUS] = 'VALID' AND [BATCH_ID] IS NULL;

    -- Composite index for status counts by file
    CREATE NONCLUSTERED INDEX [IX_MONERIS_FILE_STATUS] ON [dbo].[MONERIS_TOKENS_STAGING] ([FILE_ID], [VALIDATION_STATUS], [MIGRATION_STATUS]);
END
GO
...
BEGIN
    ...
    -- Covering index for SP_PROCESS_BATCH JOIN on MONERIS_TOKEN (eliminates key lookups)
    CREATE NONCLUSTERED INDEX [IX_PG_MONERIS_COVERING] ON [dbo].[PG_TOKENS_STAGING] ([MONERIS_TOKEN])
    INCLUDE ([PG_TOKEN], [FIRST_SIX], [LAST_FOUR], [FUNDING_METHOD], [CC_CARD_BRAND], [CC_EXP_DATE],
             [PAYMENT_METHOD_TYPE], [SOURCEOFFUNDS_NUMBER], [MONERIS2PG_MIGRATION_STATUS]);
END
GO
